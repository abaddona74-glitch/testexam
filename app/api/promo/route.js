...
export async function POST(request) {
  try {
    ...
    const DEV_PROMO_CODES = new Set(['godmode', 'showhidden', 'admin123', 'upload_privilege', 'no_copy_paste', 'copy_paste_privilege', 'correct_answers', 'sudo_access']);
    const ALLOWED = new Set([...PUBLIC_PROMO_CODES, ...DEV_PROMO_CODES]);

    const codeStr = code.toLowerCase().trim();
    if (ALLOWED.has(codeStr)) {
       const actionAliases = {
         no_copy_paste: 'copy_paste_privilege',
         correct_answers: 'show_correct'
       };
       return NextResponse.json({ success: true, action: actionAliases[codeStr] || codeStr, code: codeStr });
    }
