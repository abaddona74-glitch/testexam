                                        ...
                                        {EXAM_SCHEDULE.map((item, idx, arr) => {
                                            ...
                                            return (
                                                <div key={idx} className="relative">
                                                    {/* Arrow indicator for next exam - floating outside, doesn't affect layout */}
                                                    {isNext && (
                                                        <div className="absolute -left-9 top-1/2 -translate-y-1/2 z-10">
                                                            <img
                                                                src="/arrow.svg"
                                                                alt="Next"
                                                                className="w-7 h-7 animate-arrow-point drop-shadow-lg"
                                                            />
                                                        </div>
                                                    )}
                                                    ...
                                                    <div className={clsx(
                                                        ...
                                                        isFinished
                                                            ...
                                                            : isNext
                                                                ? "bg-white dark:bg-[#111824] border-emerald-500 shadow-md ring-1 ring-emerald-500/20"
                                                        ...
                                                        <div className={clsx(
                                                            ...
                                                            isFinished ? "bg-gray-200 dark:bg-gray-700" : isNext ? "bg-emerald-100 dark:bg-emerald-900/30" : "bg-gray-100 dark:bg-[#1f2937]"
                                                        )}>
...
function TestRunner({ test, userName, userId, sessionId, userCountry, userLocation, onFinish, onRetake, onProgressUpdate, onBack, onLeaveWithoutResult, userStars, unlockedLeagues, updateUserStars, updateUserUnlocks, spendStars, activatedCheats, soundVolume = 0.8, showTranslation = true, playClickSound, addToast }) {
    ...
    // Keep latest handlers/state for recognition callbacks.
    ...
    useEffect(() => {
        ...
        recognition.onresult = (event) => {
            ...
            if (addToast) {
                addToast('Mic', `Heard: "${transcript}"`, 'info');
            }
            ...
            else if (['next', 'next question', 'go next', 'skip', 'continue', 'keyingi', 'davom et'].includes(transcript)) {
                handleNext();
                return;
            }
